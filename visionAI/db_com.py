...
host = 'localhost'
# host = '172.16.128.3' # host ip : db server
port = 27017
# username = 'rpi'
# password = '1234'   
...
if __name__ == '__main__':
    ...
    occupied_spot = client[db_name][col_name].count_documents({})

    total_spot = 10  # Assuming total spots are 10
    available_spot = total_spot - occupied_spot

    # Step 3: Insert data into the total_parking_status collection
    parking_status = {
        'total_spot': total_spot,
        'occupied_spot': occupied_spot,
        'available_spot': available_spot,
        'current_time': current_time  # Assuming you want to record the current time
    }

    # Insert into total_parking_status collection
    client[db_name]['total_parking_status'].insert_one(parking_status)

    print("Total parking status updated successfully")
